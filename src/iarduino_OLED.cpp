...
void	iarduino_OLED::_begin(void){																					//	Параметр: отсутствует
...
void	iarduino_OLED::setFont(const uint8_t* font){																	//	Параметр:  название шрифта.
...
void	iarduino_OLED::print(int num, int x, int y, int sys){															//	Параметры: число, № колонки, № строки, система счисления.
			...
			print(k, x, y);																								//	Выводим строку k.
																														...
																														//
//		Вывод числа:																									//	Возвращаемое значение: отсутствует.
void	iarduino_OLED::print(unsigned int num, int x, int y, int sys){													//	Параметры: число, № колонки, № строки, система счисления.
			...
			print(k, x, y);																								//	Выводим строку k.
																														...
																														//
//		Вывод числа:																									//	Возвращаемое значение: отсутствует.
void	iarduino_OLED::print(double num, int x, int y, int sum){														//	Параметры: число, № колонки, № строки, количество знаков после запятой
			unsigned int i=1, j=0, k=0;																					//	Определяем временные переменные.
			j=sum; while(j){i*=10; j--;} 																				//	i=10^sum.
			print(int(num), x, y);																						//	Выводим целую часть числа.
...
#else																													//
//		Вывод числа:																									//	Возвращаемое значение: отсутствует.
void	iarduino_OLED::print(int8_t num, int x, int y, int sys){														//	Параметры: число, № колонки, № строки, система счисления.
			print(int32_t(num), x, y, sys);																				//	Преобразуем int8_t в int32_t и вызываем функцию вывода числа print(int32_t).
}																														//
																														//
//		Вывод числа:																									//	Возвращаемое значение: отсутствует.
void	iarduino_OLED::print(uint8_t num, int x, int y, int sys){														//	Параметры: число, № колонки, № строки, система счисления.
			print(uint32_t(num), x, y, sys);																			//	Преобразуем uint8_t в uint32_t и вызываем функцию вывода числа print(uint32_t).
}																														//
																														//
//		Вывод числа:																									//	Возвращаемое значение: отсутствует.
void	iarduino_OLED::print(int16_t num, int x, int y, int sys){														//	Параметры: число, № колонки, № строки, система счисления.
			print(int32_t(num), x, y, sys);																				//	Преобразуем int16_t в int32_t и вызываем функцию вывода числа print(int32_t).
}																														//
																														//
//		Вывод числа:																									//	Возвращаемое значение: отсутствует.
void	iarduino_OLED::print(uint16_t num, int x, int y, int sys){														//	Параметры: число, № колонки, № строки, система счисления.
...
void	iarduino_OLED::print(double num, int x, int y, int sum){														//	Параметры: число, № колонки, № строки, количество знаков после запятой
			...
			if(sum){																									//	Если требуется вывести хоть один знак после запятой, то ...
				...
				if(j==0){while( sum  ){print("0"); sum--;}          }													//	Если полученное целое число = 0, то выводим sum раз символ 0.
																														...
																														//
//		Вывод числа:																									//	Возвращаемое значение: отсутствует.
void	iarduino_OLED::print(int32_t num, int x, int y, int sys){														//	Параметры: число, № колонки, № строки, система счисления.
			int8_t i=2; int32_t j=1;			while(num/j){j*=sys; i++;}												//	Определяем количество разрядов числа (i = количество разрядов + 2, j = множитель кратный основанию системы счисления)
			char k[i]; i--; k[i]=0; i--;		if(num>0) {k[i]=0; i--;}												//	Создаём строку k из i символов и добавляем символ(ы) конца строки.
			uint32_t n=num<0?num*-1:num;		while(i)  {k[i]=_ItoAa(n%sys); n/=sys; i--;}							//	Заполняем строку k.
												if(num>=0){k[i]=_ItoAa(n%sys);}else{k[i]='-';}							//	Добавляем первый символ (либо первая цифра, либо знак минус).
			print(k, x, y);																								//	Выводим строку k.
																														...
																														//
//		Вывод числа:																									//	Возвращаемое значение: отсутствует.
void	iarduino_OLED::print(uint32_t num, int x, int y, int sys){														//	Параметры: число, № колонки, № строки, система счисления.
			int8_t i=1; uint32_t j=1;			while(num/j){j*=sys; i++;} if(num==0){i++;}								//	Определяем количество разрядов числа (i = количество разрядов + 1, j = множитель кратный основанию системы счисления)
			char k[i]; i--; k[i]=0;				while(i){k[i-1]=_ItoAa(num%sys); num/=sys; i--;}						//	Определяем строку k из i символов и заполняем её.
			print(k, x, y);																								//	Выводим строку k.
}																														//
...
void	iarduino_OLED::print(char sym, int col, int str){																//	Параметры: символ, № колонки, № строки.
...
void	iarduino_OLED::drawImage(const uint8_t* image, int x, int y, uint8_t mem){										//	Параметры: изображение, № колонки, № строки, тип памяти.
...
uint8_t	iarduino_OLED::getImageWidth(const uint8_t* image, uint8_t mem){												//	Параметры: изображение, тип памяти.
...
uint8_t	iarduino_OLED::getImageHeight(const uint8_t* image, uint8_t mem){												//	Параметры: изображение, тип памяти.
			...
			//	Преобразуем текст из кодировки WINDOWS-1251:															//
				case TXT_WIN1251:																						//
					while (charThis> 0   &&numIn    <0xFF){																//	Если код текущего символа строки StrIn больше 0 и № текушего символа строки StrIn меньше 255, то ...
						...
						if(charThis>=0xF0                ){StrOut[numOut]=charThis-0x10;}else							//	Если код текущего символа равен 240...255, значит это буква «р»...«я» требующая преобразования к коду 224...239
